...
//ignore_user
$.getJSON("https://sk-knower.com/function/sql_cfm_twitchchatblocklist.php",function(data){
...
if(set_number=='0'){
	...
	if ($('#chat_down').css('display') == 'none') {
		directbottom='true';
		skchatscroll.scroll(9999999999999999);
	}else{
		directbottom='true';
	}
	if(scrollstate<=250||directbottom=='false'){
...
setTimeout(function(){
	...
	if(scrollstate<=99||directbottom=='false'){
		if(prepend=='true'&&nicochat!='on' || prepend=='on'&&nicochat!='on'){
			skchatscroll.scroll(0);
			}else{
			skchatscroll.scroll(9999999999999999);
...
if(image=='340'){
	...
	if(scrollstate<=340||directbottom=='false'){
		if(prepend=='true'&&nicochat!='on' || prepend=='on'&&nicochat!='on'){
			skchatscroll.scroll(0);
			}else{
			skchatscroll.scroll(9999999999999999);
...
$.getJSON("https://sk-knower.com/sklive_api/read/skchat_set.php?showset=public", function (setarray) {
	...
	$.each(set, function (index, list) {
	  ...
	  if(list.about!=''){
		...
		$("#icon-set-"+list.name).append('<div class="aboutset"><div style="width: 100%;padding: 2px 0px;">'+list.about+'</div>'+setaction+'</div>');
	  }
	  loadset(list.name)
...
if(specialclass=='successconnect'){
	if(firstloadchatjs=='true'){
		googletag.cmd.push(function() { googletag.display('div-gpt-ad-1634122781818-0'); });
	}else{
...
if (data[1] == 'history' ) {
	if(reconnectstatee!='true'){	
		if(firstloadchatjs=='true'){
			showMsg( "<div id='div-gpt-ad-1634122781818-0' style='min-width: 300px; min-height: 250px;text-align: center;'>", 'server', '', '','', '', 'successconnect','true');
			firstloadchatjs='false';
		}else{
			showMsg( '<ins class="adsbygoogle" style="display:inline-block;width:336px;height:280px" data-ad-client="ca-pub-7085319547010052" data-ad-slot="7951613088"></ins>', 'server', '', '','', '', 'successconnect','true');
		}
	$(".loadinghistory").hide();
...
showMsg('<span class="skClass"><img src="//live.sk-knower.com/logo/sklive_light.png" style="height: 25px;">聊天室載入中...</span>', 'server');
$('#top-box').css('height',$(window).innerHeight()+'px');
...
var b = div.textContent;
	...
	if (b.match(/youtu/g)&&!b.match(/img/g)&&!b.match(/jpg/g)) {
		if (b.match(/watch?/g)) {
			b = b.replace('https://m.youtube.com/watch?', '');
			...
			b = b.replace('https://www.','').replace('https://','').replace('m.youtube.com/watch?v=', '').replace('youtube.com/watch?v=', '').replace('youtu.be/', '').replace('youtube.com/embed/', '').replace('youtube.com/shorts/', '').replace('youtube.com/live/', '').replace('&feature=share', '');
		  }
		...
		var e = '<a class="skpopimg skimg" href="'+b+'" target="_blank"><video id="' + number + '" onclick="vodgif('+number+');" autoplay="true" loop="true" playsinline="true" muted="true" style="max-height: 200px;  max-width: 250px;" src="' + b + '"></video></a>';
                              ...
                              if (end > start && end >= 0 && start >= 0) {
                                ...
                                if (b.match(/facebook/g)) {
								  ...
								  var e = '<div><div style="padding: 5px;background: rgba(0, 0, 0, 0.45098039215686275);color: white;border-radius: 5px 5px 0px 0px;">Facebook <i class="facebookclosevideo material-icons" data-fbid="'+fbvideoid+'" class="material-icons" style="cursor:pointer;font-size: 20px;float: right;" >close</i> <i class="material-icons" data-fancybox data-type="iframe" data-src="https://www.facebook.com/plugins/video.php?href='+b+'&amp;show_text=0&amp;width=300&height=200" href="javascript:;" style="cursor:pointer;font-size: 20px;float: right;">open_in_new</i></div><div style="min-width: 250px;text-align: center;line-height: 200px;height: 200px;box-shadow: none;color: white;position: relative;overflow: hidden;background: black;cursor: pointer;" data-fbid="'+b+'" class="facebookvideo '+fbvideoid+'"><div style="Background-image: url();position: absolute;color: white;z-index: 0;width: 100%;height: 100%;opacity: 0.5;background-size: contain;background-repeat: no-repeat;background-position: center;"></div><div>播放</div></div></div>';
									...
									var e = '<blockquote class="twitter-video" style="width:100%;" data-lang="zh-tw"><a href="'+ b+'"></a></blockquote>';
									var twitter='true';
								} else if (b.match(/.webm/g) || b.match(/.mp4/g)) {
									var number = Math.floor((Math.random() * 100000) + 1);
									var e = '<a class="skpopimg skimg" href="'+b+'" target="_blank"><video id="' + number + '" autoplay="" loop="" playsinline="true" muted="" style="max-height: 200px;  max-width: 250px;" src="' + b + '"></video></a>';
